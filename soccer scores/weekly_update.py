import requests
from bs4 import BeautifulSoup
import re
import pandas as pd
import numpy as np
from time import sleep
from datetime import datetime
from selenium import webdriver
import random
import sqlite3
import os

df_help = pd.read_excel('config.xlsx')
df_help['active'] = df_help['active'].astype(str)
# df_help = df_help[df_help.active == '1']
names_help = []
for link_help in df_help['link']:
    link_help = link_help.strip('/')
    coun = link_help.split('/')[-3]
    leag = link_help.split('/')[-2]
    leag = leag.replace('-', '')
    coun = coun.replace('-', '')
    fin = f'{coun}{leag}'
    names_help.append(fin)
# print(names_help)
# c_path = os.path.dirname(__file__)
# os.chdir(c_path)

days = []
today = datetime.today().weekday()
for x in range(7, -2, -1):
    days.append(x-today)
home_links = []

for x in days:
    home_links.append(f'https://m.diretta.it/?d={x}')


contents_links = []
for home_link in home_links:
    try:
        page = requests.get(home_link)
        sleep(1)
        if page.status_code == 200:
            home_soup = BeautifulSoup(page.text, 'lxml')
            content = home_soup.find('div', {'id': 'score-data'})
            contents_links.append(content)
    except:
        print(home_link)

links = []
for content in contents_links:
    all_leagues = str(content).split('<h4>')
    all_1 = []
    c = 0
    for xx in all_leagues:
        xx = xx.rstrip('<br/>')
        xx = '<h4>' + xx + '>'
        all_1.append(xx)

for xx in all_1:
    try:
        title = title.split('-')[0]
        title = BeautifulSoup(xx, 'lxml').find('h4').text
        title = title.replace('-', '')
        title = title.replace('.', '')
        title = title.replace(' ', '')
        title = title.replace(':', '')
        title = title.lower()
        if title in names_help:

            if '<br/>' in xx:
                matchesx = xx.split('<br/>')
                for matchx in matchesx:
                    soupx = BeautifulSoup(matchx, 'lxml')
                    link = soupx.find('a')['href']
                    link = 'https://www.diretta.it' + link + \
                        '#informazioni-partita/informazioni-partita'
                    links.append(link)
            else:
                soupx = BeautifulSoup(xx, 'lxml')
                link = soupx.find('a')['href']
                link = 'https://www.diretta.it' + link + \
                    '#informazioni-partita/informazioni-partita'
                links.append(link)
    except:
        pass


countries = []
leagues = []
dates = []
times = []
homes = []
aways = []
fts = []
hts = []
first_values = []
first_dirs = []
x_values = []
x_dirs = []
second_values = []
second_dirs = []

driver = webdriver.Chrome('chromedriver.exe')
driver.maximize_window()

soups = []
for c, match_link in enumerate(links):
    print(f'match: {c+1}/{len(links)}')
    sleep(random.random())
    driver.get(match_link)
    sleep(random.random()*1.2)
    soups.append(BeautifulSoup(driver.page_source, 'lxml'))
    c += 1
    if c == 3:
        break


for c, soup in enumerate(soups):

    try:
        try:
            three_ods_raw = soup.find_all('div', class_='cellWrapper')

            try:
                first_value = three_ods_raw[0]['title']
                first_value = re.findall('\d+.\d+', first_value)[-1]
                first_dir = three_ods_raw[0]['title']
                first_dir = re.findall(r'\[\w\]', first_dir)[0]
                first_dir = first_dir.replace('[', '')
                first_dir = first_dir.replace(']', '')
                first_dir = first_dir.replace('u', 'UP')
                first_dir = first_dir.replace('d', 'DOWN')
            except IndexError:
                first_value = three_ods_raw[0].text
                first_value = re.findall(
                    '\d+.\d+', first_value)[-1]
                first_dir = '-'

            try:
                x_value = three_ods_raw[1]['title']
                x_value = re.findall('\d+.\d+', x_value)[-1]

                x_dir = three_ods_raw[1]['title']
                x_dir = re.findall(r'\[\w\]', x_dir)[0]
                x_dir = x_dir.replace('[', '')
                x_dir = x_dir.replace(']', '')
                x_dir = x_dir.replace('u', 'UP')
                x_dir = x_dir.replace('d', 'DOWN')
            except IndexError:
                x_value = three_ods_raw[1].text
                x_value = re.findall(
                    '\d+.\d+', x_value)[-1]
                x_dir = '-'

            try:
                second_value = three_ods_raw[2]['title']
                second_value = re.findall('\d+.\d+', second_value)[-1]
                second_dir = three_ods_raw[2]['title']
                second_dir = re.findall(r'\[\w\]', second_dir)[0]
                second_dir = second_dir.replace('[', '')
                second_dir = second_dir.replace(']', '')
                second_dir = second_dir.replace('u', 'UP')
                second_dir = second_dir.replace('d', 'DOWN')
            except IndexError:
                second_value = three_ods_raw[2].text
                second_value = re.findall(
                    '\d+.\d+', second_value)[-1]
                second_dir = '-'

        except:
            first_value = '-'
            first_dir = '-'
            x_dir = '-'
            x_value = '-'
            second_dir = '-'
            second_value = '-'

#         first_value = three_ods_raw[0]['title'][7:]
#         first_dir = three_ods_raw[0]['title'][5]
#         if first_value == '1.04':
#             print(c)

#         x_value = three_ods_raw[1]['title'][7:]
#         x_dir = three_ods_raw[1]['title'][5]

#         second_value = three_ods_raw[2]['title'][7:]
#         second_dir = three_ods_raw[2]['title'][5]

        country_league = soup.find(
            'span', class_='tournamentHeader__country').text
        country = country_league.split(':')[0].strip()
        league = country_league.split(':')[1].strip()
        countries.append(country)
        leagues.append(league)

        date_time = soup.find(
            'div', class_='duelParticipant__startTime').text
        date_ = date_time.split()[0]
        time_ = date_time.split()[1]
        dates.append(date_)
        times.append(time_)

        home = soup.find(
            'div', class_='duelParticipant__home').text.strip()
        homes.append(home)
        away = soup.find(
            'div', class_='duelParticipant__away').text.strip()
        aways.append(away)

        full_time = soup.find(
            'div', class_='duelParticipant__score').text.strip('Finale')
        full_time = full_time.strip('\xa0')
        try:
            full_time = re.findall('\d+-\d+', full_time)[0]
        except:
            full_time = '-'
        # print(full_time)
        if full_time == '-':
            half_time = '-'
        else:
            try:
                half_time = soup.find(
                    'div', class_='smv__incidentsHeader section__title').text.replace(' ', '')
                half_time = re.findall('\d+-\d+', half_time)[-1]
                # print(half_time)
            except:
                half_time = '-'
                # half-time-error
                print(c, 'half_time error')

        first_values.append(first_value)
        first_dirs.append(first_dir)

        x_values.append(x_value)
        x_dirs.append(x_dir)

        second_values.append(second_value)
        second_dirs.append(second_dir)

        fts.append(full_time)
        hts.append(half_time)
    except:
        print(c, 'full-error')
df = pd.DataFrame({
    'country': countries,
    'league': leagues,
    'date': dates,
    'time': times,
    'home': homes,
    'away': aways,
    'FT': fts,
    'HT': hts,
    '_1_dir': first_dirs,
    '_1': first_values,
    'x_dir': x_dirs,
    'x': x_values,
    '_2_dir': second_dirs,
    '_2': second_values
})

f_name = str(datetime.today().date())

# cr.execute(
#     f'create table if not exists {tablename_sql} (country text, league text, date text, time text, home text,away text, FT text, HT text, _1_dir text, _1 text, x_dir, x, _2_dir text, _2 );')
# df.to_sql(f'{tablename_sql}', conn, if_exists='append',
#             index=False, index_label=False)

df.to_excel(f'results\\weekly\\{f_name}.xlsx',
            index=False)


driver.close()
# conn.commit()
# cr.close()
# conn.close()
input('\nDone.....\nPress Enter to close')
